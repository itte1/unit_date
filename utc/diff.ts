...
export function diff(left: Date, right: Date, unit?: DateUnit): number {
  switch (unit) {
    case 'years': return left.getUTCFullYear() - right.getUTCFullYear()
    case 'months': return (left.getUTCFullYear() - right.getUTCFullYear()) * 12 + left.getUTCMonth() - right.getUTCMonth()
    default: {
